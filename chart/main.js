/* globals template_data, Promise, vega, array_Object_keys, slug, vegaEmbed, Papa */
...
Promise.all([
  ...
  fetch("examples.json")
    ...
    .then(function (res) {
      ...
      examples_json_chart = examples_json_chart.map(function (item) {
...
function chartDidRender() {
  ...
  if (templates) {
    ...
    form_default_setter();
    $("body").on("change", "#customize_chart", function (event) {
      if (event.target.dataset["delete"]) {
        event.target.dataset["delete"].split("@").map(function (del_prop) {
          _.unset(
            model.vega_spec,
            event.target.dataset["path"].split(".").slice(0, -1).join(".") +
              "." +
              del_prop,
          );
        });
  ...
  Promise.all(
    ...
    .then(function (tmpls) {
      ...
      $('a[data-bs-toggle="tab"]', $("#stages")).on(
        "shown.bs.tab",
        function (e) {
          $("h6", $(e.relatedTarget))
...
function drawCopyPasteBlock(response) {
  ...
  if (!data_mapper) $(".render-chart").remove();
...
$("body")
  ...
  .on("click", ".save-mapper", function () {
    ...
    var new_mapper = Object.keys(data_mapper).reduce(function (
      acc,
      current_col,
    ) {
...
function on_cols_fetch(res) {
  var fh_dataset = res[0],
    gallery_dataset = res[1];
...
$("body").on("click", ".palette", function (event) {
  ...
  var prop_field_path =
    scale_prop_path.split(".").slice(0, -1).join(".") + ".field";
