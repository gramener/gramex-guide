...
function main(recognition, synth) {
  ...
  $.getJSON("suggestion").done(function (data) {
    data.forEach(function (d) {
      $(".sample-questions").append(
  ...
  recognition.onresult = function (event) {
    ...
    $.getJSON("get_answer", { q: text }).done(function (result) {
      $(".speak-btn-text").text("Ask me");
      ...
      else if (result.similarity < 0.75) $(".did-you-mean").show();
      if (synth)
