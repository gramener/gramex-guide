        ...
        _$:
          {
            ...
            "values":
              [
                {
                  "on": "vegam-custloadtimetrend",
                  "title": "Custom Metric Time",
                },
                {
                  "on": "vegam-custloadtimerequeststrend",
...
var config = {
  ...
  viewsConfig: [
    ...
    {
      type: "viz",
      url: "query/aggD/custloadtimetrend/",
      on: ".vegam-custloadtimetrend",
      viz: [
        { data: null, options: { types: { time: "date" } } },
        {
    ...
    {
      type: "viz",
      url: "query/aggD/custloadtimerequeststrend/",
      on: ".vegam-custloadtimerequeststrend",
      viz: [
        { data: null, options: { types: { time: "date" } } },
        { apply: "area", x: "time", y: "requests", props: { fill: "#cc95ff" } },
...
logviewer:
  ...
  YAMLURL: $YAMLURL/log/
  LOGVIEWER_FORMHANDLER_QUERIES:
    slow-loading-customview: SELECT COUNT(DISTINCT uri) AS value
      FROM {table}
      WHERE slow == 1 {where}
```
...
- "@class": d-flex flex-wrap my-4
  _$: [
      ...
      {
        "tpl": "lv-kpi",
        "on": "slow-loading-customview",
        "title": "Custom Slow Loading View",
      },
    ]
```
...
viewsConfig: [
    ...
    {
      type: "kpi",
      url: "query/aggD/slow-loading-customview/",
      on: ".slow-loading-customview",
      formatter: d3.format(',
      .2d'),
    },
