function condense(result) {
  ...
  if (!field || !field.user) return result;
  result = {
    id_str: field.id_str,
  ...
  if ("sentiment" in field) result.sentiment = field.sentiment;
  return result;
}
  ...
  if (text.match(output_regex))
    if (text.match(/\$.when/)) {
      ...
      eval(text).then(function (res1, res2) {
        var result = [];
        for (var r of [res1, res2]) result.push(condense(r));
      ...
      eval(text).always(function (result) {
        replace(
          element,
          output_regex,
          JSON.stringify(condense(result), null, 2),
        );
      });
    }
  if (pre.length > 0) {
